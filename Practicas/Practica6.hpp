...
typename GrafoP<tCoste>::vertice pseudocentro(GrafoP<tCoste>& g, tCoste& diametro)
{
    ...
    {
        if(sumaMayores(m[v])<sumaMin) //Seleccionamos aquel nodo cuya suma de costes a sus nodos 
        //mas alejados sea minima
        {
 ...
 matriz<bool>& carreteras, typename GrafoP<tCoste>::vertice capital)
{
    ...
    for(i=0;i<ciudades.size();++i)
    {
        ...
        {
            g[ciudades[i]][destino]=GrafoP<tCoste>::INFINITO;
            g[destino][ciudades[i]]=GrafoP<tCoste>::INFINITO;
        }
    }
